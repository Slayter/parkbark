...
class Survey_PoopBags extends Component {
  ...
  saveFormData(updateValue) {
      ...
      if(this.props.suggestPark) {
        return Actions.surveyHikingTrails({suggestPark: true})
      }
      Actions.surveyHikingTrails();
  }
  ...
  onClosePress(formData) {
      const updateValue = {};
      updateValue.title = 'poop_bags';
      updateValue.value = this.state.formData.poop_bags;
      this.props.dispatch({type: 'UPDATE_SURVEY', state: updateValue});
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                <Text style={styles.question}>Are there <B>poop bags</B> provided?</Text>
                       <Form
                           ref='surveyFormPoopBags'
...
export default connect(mapStateToProps)(Survey_PoopBags);
