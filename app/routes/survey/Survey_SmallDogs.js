...
class Survey_SmallDogs extends Component {
  constructor(props) {
    super(props);
  ...
  saveFormData(updateValue) {
    ...
    if (this.props.suggestPark) {
      return Actions.surveyDrinkingWater({ suggestPark: true });
    }
  ...
  onClosePress = () => {
    const updateValue = {};
    updateValue.title = 'small_dogs';
    updateValue.value = this.state.formData.small_dogs;
    this.props.dispatch({ type: 'UPDATE_SURVEY', state: updateValue });
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Text style={styles.question}>Is there specific area for <B>small dogs</B>?</Text>
        <Form
          ref="surveyFormSmallDogs"
...
export default connect(mapStateToProps)(Survey_SmallDogs);
